            ...
            <div class="admin-content-right">
                ...
                <div class="row">
                        <div class="col d-flex justify-content-end">
                            <div class="box-right">
                                <div class="withdraw">
                                    <a href="{{route('admin.createchalan')}}" class="button button-outline-primary button-round">Create Chalan</a>
                                </div>
                            </div>
                        </div>
                   
                </div>
                <h2 class="box-heading mt-3">Chalan List</h2>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @foreach($chalans as $chalan)
                                <tr>
                                    <td>{{ $chalan->user->name }}</td> <!-- Assuming 'student' relationship returns user -->
                                    <td>{{ $chalan->rule->name }}</td> <!-- Assuming 'rule' relationship returns the rule related to chalan -->
                                    <td>{{ $chalan->status }}</td>
                                    <td> @if($chalan->status !== "pending")
                                        &nbsp;   <a href="{{ asset('storage/' . $chalan->payment_proof) }}" target="_blank">View Proof</a>
                                        @else
                                          &nbsp;  Not provided
                                        @endif</td>
                                    {{-- <td>{{ $chalan->payment_proof ?? 'N/A' }}</td> <!-- Display 'N/A' if no proof --> --}}
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
